...
def get_segmented_frames(args, video_input, output=None, resolution=None, human_filter=False):
    cfg = setup_cfg(args)
    ...
    frames_info = []
    frames = []
    for i, (frame, model_output) in enumerate(tqdm.tqdm(process_video(model, video, resolution=resolution), total=num_frames)):
        if output:
            ...
            for i in range(len(model_output['instances'])):
                if model_output['instances'][i].pred_classes.item() == 0:
                    instances = model_output['instances'][i]
                else:
                    continue
            
            vis_frame = v.draw_instance_predictions(instances.to("cpu"))
